...
Cmac=0.025;
aspectratio=13.45794;
alpha_o=deg2rad(0.5);
gamma=deg2rad(20); % maximum flapping
eta=0.98; % leading edge suction efficiency
rho= 0.92239; % density of air(kg/m3) at 2400m
theta_a=deg2rad(7.5); % pitching angle of flapping angle with respect to U(rad)
f=1.2; % flapping frequency (Hz)
U=13.4112; % forward velocity (m/s)
mew=0.000017859; % (coefficient of viscocity(Ns/m2) or in (kg/ms)) at 2400m
dt=1/(20*f); %time interval
dy=0.27432;%section width
ddy=dy*10;
span = [0.1143 0.3429 0.5715 0.8001 1.0287 1.2573 1.4859 1.7145 1.9431 2.1717 2.4003 2.6289 2.8575]
c = [0.74422 0.60706 0.51562 0.45212 0.41656 0.41148 0.42418 0.36322 0.30988 0.28956 0.23114 0.127 0]
beta=0:0.82021:9.84252; % deg/meter
for z=1:length(beta)
